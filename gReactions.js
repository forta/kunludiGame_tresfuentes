...
let initReactions =  function  (reactions, primitives) {
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if ((primitives.IT_GetLoc (primitives.IT_X("hechicera")) == primitives.PC_GetCurrentLoc()) || (primitives.IT_GetLoc (primitives.IT_X("vieja")) == primitives.PC_GetCurrentLoc()) ) { // vieja/bruja aquí
				...
				primitives.GD_CreateMsg ("es", "DLG_no_toques", "En mi presencia, ¡ni se te ocurra tocar ese instrumento del demonio!");
				primitives.CA_QuoteBegin (PNJ, "DLG_no_toques");
				primitives.GD_CreateMsg ("es", "%o1_no_te_deja_tocar", "%o1 te intimida tanto, que ni te atreves a intentarlo.");
			...
			primitives.GD_CreateMsg ("es", "practicar_flauta_0", "fiu, fiu, la conchambrosa flauta sólo hace un ruido sin gracia, apenas si sabes colocar los dedos en los agujeros.<br/>");
			primitives.GD_CreateMsg ("es", "practicar_flauta_1", "Después de mucho practicar, consigues ir asociando sonidos a digitaciones de dedos.<br/>");
			primitives.GD_CreateMsg ("es", "practicar_flauta_2", "Te atreves a tocar de oído algunas pequeñas combinaciones.<br/>");
			primitives.GD_CreateMsg ("es", "practicar_flauta_3", "Ahora intentas tocar una melodía más larga que oiste una vez.<br/>");
			primitives.GD_CreateMsg ("es", "practicar_flauta_4", "La práctica es la madre de la maestría, ahora hasta podrías ganarte unos cuarto tocando en la calle.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "cambio_punto_de_vista_a_%o1", "<br/><b>Cambio de protagonista:</b> %o1<br/><br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "al_pasar_delante_figura", "Al pasar por delante de %o1 no puedes dejar de observarla: ");
			primitives.GD_CreateMsg ("es", "cazador_atraviesa_catarata", "Con mucho esfuerzo, atraviesas la muralla de agua de la catarata.<br/>");
			primitives.GD_CreateMsg ("es", "VR_dragona_atraviesa_catarata_1", "Convertida en dragona, ");
			primitives.GD_CreateMsg ("es", "VC_dragona_atraviesa_catarata_1", "Convertida en mostruo marino, ");
			primitives.GD_CreateMsg ("es", "dragona_atraviesa_catarata_2", "atraviesas la muralla de agua de la catarata sin dificultad.<br/>");
			primitives.GD_CreateMsg ("es", "VR_reflejo_catarata", "Te ves reflejada en la fuente verde... eres tú, pero no eres tú... sólo reconoces de ti misma los ojos de serpiente. Con este cuerpo de mujer no podrías atravesar la cascada, así que por ahora no lo intentas... pero hay una dragona dentro de ti.<br/>");
			primitives.GD_CreateMsg ("es", "VC_reflejo_catarata", "Te ves reflejada en la fuente verde y reconoces tus ojos de serpiente. No te arriesgas a poner en riesgo la vida de tu hijo... aún no.<br/>");
			...
			if ( (par_c.loc == primitives.IT_X("fuente_verde")) && (par_c.directionId == "in") )  {
				if (primitives.PC_X() == primitives.IT_X("cazador")) {
					if ( (primitives.IT_GetAttPropValue (primitives.PC_X(), "generalState", "state") == "4")  && // cazador no ha bebido
						...
						primitives.GD_CreateMsg ("es", "cazador_no_cruza_sin_pócima", "Te acuerdas de que no has tomado la pócima y te parece una tontería avanzar sin haberla tomado.<br/>");
			...
			if (par_c.loc == primitives.IT_X("tierras_sur") && (par_c.directionId == "in")) {
				if (primitives.PC_X() == primitives.IT_X("vagabunda")) {
					if (primitives.IT_GetAttPropValue (primitives.IT_X("tierras_sur"), "generalState", "state") == "0") { // bloqueada la entrada
						primitives.GD_CreateMsg ("es", "cueva_bruja_innaccesible_vagabunda", "Una grieta en la base de la montaña, al sur, parece indicar una entrada en la montaña, pero una robusta planta llena de espinas no te permite entrar. Desistes.<br/>");
						...
						if (primitives.IT_GetAttPropValue (primitives.PC_X(), "generalState", "state") == "5") { // 5: bebió pócima: dragona no puede volver a la cueva de la bruja
							primitives.GD_CreateMsg ("es", "dragona_no_pasa", "Miras la grieta... miras cómo ha crecido tu cuerpo... y desistes sin intentarlo.<br/>");
				...
				if ( (primitives.IT_GetLoc (primitives.IT_X("diamante")) == primitives.PC_X ()) &&
					...
					primitives.GD_CreateMsg ("es", "flautista_aparece", "De entre unos matorrales aparece el flautista.<br/>");
					primitives.GD_CreateMsg ("es", "DLG_flautista_quita_diamante", "No creerías que te iba a dejar llevarle la valiosa piedra a la vieja antes de que ejerza su magia para mí.");
					primitives.GD_CreateMsg ("es", "flautista_se_va", "El flautista te arrebata el diamante y desaparece de tu vista.<br/>");
			...
			if  (par_c.loc == primitives.IT_X("cueva_bruja")) {
				if (primitives.IT_GetIsLocked (par_c.loc, par_c.directionId)) {
					...
					primitives.GD_CreateMsg ("es", "No_puedes_salir_de_cueva_bruja", "Has crecido mucho, ¡ya no cabes por la grieta de salida y no puedes salir de la cueva!<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "no_llevas_portaagua %o1", "No tienes con qué coger %o1");
			primitives.GD_CreateMsg ("es", "recuerdas_regla %s1", "Recuerdas la regla número %s1 que te dijo la hechicera.<br/>");
			primitives.GD_CreateMsg ("es", "contenido_cuenco_no_cambia", "El contenido del cuenco no cambia.<br/>");
			primitives.GD_CreateMsg ("es", "regla1", "El líquido verde simboliza a los elfos y ofrece resistencia a que le pongas otros líquidos encima. Como los elfos, desean ser siempre los últimos en llegar a las batallas y decir que gracias a ellos se ganó.<br/>");
			primitives.GD_CreateMsg ("es", "efecto_regla1", "El líquido verde empieza a girar dentro del cuenco. Te himnotiza, parece formar forma de elfo y se eleva por los aires, dejando el cuenco vacío.<br/>");
			primitives.GD_CreateMsg ("es", "regla2", "Los líquidos verde y dorado representan los bosques de los elfos y el oro de los enanos. Al igual que el agua y el aceite, no se mezclan a solas ni por casualidad.<br/>");
			primitives.GD_CreateMsg ("es", "efecto_regla2", "El líquido dorado empieza a burbujear y calentarse y se evapora antes de que llegues a coger el agua verde.<br/>");
			primitives.GD_CreateMsg ("es", "regla3", "Los líquidos verde y dorado representan los bosques de los elfos y la sangre de dragón. A los dragones no les gusta ser montados por los elfos.<br/>");
			primitives.GD_CreateMsg ("es", "efecto_regla3", "El cuenco empieza a moverse como un caballo que no quiere ser domado, y el agua acaba cayendo al suelo.<br/>");
			primitives.GD_CreateMsg ("es", "liquido_contenido %o1 %o2", " %o1 tiene %o2.<br/>");
			primitives.GD_CreateMsg ("es", "efecto_desconocido", "Por algún motivo desconocido, ¡la mezcla de líquidos reacciona!<br/>");
			...
			// pájaros
			primitives.GD_CreateMsg ("es", "pájaros_no_capturables", "Los pájaros están aburridos de que los caces, te huelen a la milla y salen volando.<br/>");
			primitives.GD_CreateMsg ("es", "VC_pájaros_son_capturados", "Lanzas la honda y dos de ellos quedan atrapados en ella; el resto sale volando.<br/>");
			primitives.GD_CreateMsg ("es", "VR_pájaros_son_capturados", "Lanzas la camisa y dos de ellos quedan atrapados en ella; el resto sale volando.<br/>");
			primitives.GD_CreateMsg ("es", "pájaros_no_son_capturados", "Los intentas cazar, pero son más rápido que tú. Quizás, lanzándoles algo encima...<br/>");
			primitives.GD_CreateMsg ("es", "pescados_no_capturables", "Los peces es como si te sintieran desde lejos, y nada más aproximarte se alejan sin que puedas ni verlos.<br/>");
			primitives.GD_CreateMsg ("es", "pescados_son_capturados", "Con el anzuelo y un palo haces una caña improvisada y consigues pescar uno.<br/>");
			primitives.GD_CreateMsg ("es", "pescados_no_son_capturados", "Los intentas pescar, pero son más rápido que tú. Quizás, con alguna cosa...<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "pájaros_libres", "Los pájaros beben plácidamente junto a la charca roja.<br/>");
			primitives.GD_CreateMsg ("es", "pájaros_capturados", "Los pájaros que cazaste.<br/>");
			primitives.GD_CreateMsg ("es", "pescados_libres", "Los pescados se mueven cerca del margen del río.<br/>");
			primitives.GD_CreateMsg ("es", "pescados_capturados", "Los pescados que pescaste.<br/>");
			primitives.GD_CreateMsg ("es", "VR_figura_roja", "La figura está formada por dos pequeños dragones rojos entrelazados que indican el camino del este. Bajo la figura se lee una inscripción con sus nombres: Laila y Kuko.<br/>");
			primitives.GD_CreateMsg ("es", "VC_figura_roja", "La figura representa un dragón rojo que mira hacia el este.<br/>");
			...
			if (par_c.item1Id == "basura") {
				...
				primitives.GD_CreateMsg ("es", "al_examinar_basura_aparece_cuenco", "Removiendo entre ramas y polvo, descubres un viejo cuenco");
				...
				if (usr.getVersion() == "VR") {
					primitives.GD_CreateMsg ("es", "junto_con_cuenco_aparece_camisa", " y una camisa sucia.<br/>");
			...
			// espejo
			primitives.GD_CreateMsg ("es", "VC_vagabunda_ante_espejo", "Qué extraño te resulta verte con esa enorme barriga, pero sigues teniendo tus ojos de serpiente.<br/>");
			primitives.GD_CreateMsg ("es", "VR_vagabunda_ante_espejo", "Qué extraño te resulta verte con esta figura humana. Sigues teniendo tus ojos de serpiente.<br/>");
			primitives.GD_CreateMsg ("es", "vagabunda_ante_espejo_ve_diamante", "Recordando el aviso del flautista, observas el espejo con mayor atención y descubres, sin saber cómo, hay una piedra preciosa dentro del espejo. Alargas la mano y coges el diamante, accesible sólo a seres como tú.<br/>");
			primitives.GD_CreateMsg ("es", "vagabunda_ante_espejo_no_ve_diamante", "Por un momento te parece ver un refrejo extraño, pero tuvo que haber sido tu imaginación..<br/>");
			primitives.GD_CreateMsg ("es", "cazador_ante_espejo", "Te notas más viejo de lo que recordabas, como si hubieran pasado años desde que fuiste al mundo submarino en búsqueda del anzuelo para tu hermano.<br/>");
			...
			if (par_c.item1Id == "flautista") {
				primitives.GD_CreateMsg ("es", "flautista_tocando", "El flautista, tocando animadamente su flauta.");
				primitives.GD_CreateMsg ("es", "flautista_sin_flauta", "Como el flautista ya no tiene flauta... canta animadamente.");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "DLG_acepta_petición_porque %o1", "Vale, pero sólo porque antes me diste %o1.");
			...
			primitives.GD_CreateMsg ("es", "DLG_da_%o1", "Vale, toma %o1.");
			...
			// pedir a vieja
			primitives.GD_CreateMsg ("es", "DLG_no_da_%o1", "¿Y por qué tendría que darte yo %o1?");
			...
			// pedir a hechicera
			primitives.GD_CreateMsg ("es", "DLG_no_da_%o1_porque_falta_%o2", "Si quieres %o1 deberías traerme %o2.");
			primitives.GD_CreateMsg ("es", "DLG_coge_de_%o1", "Sírvete tú mismo de %o1.");
		...
		reaction: function (par_c) {
			...
			if (par_c.item2Id == "cazador") {
				if (par_c.item1Id == "honda") {
					primitives.GD_CreateMsg ("es", "DLG_preguntar_a_cazador_por_onda", "Gracias a ella, he cazado innumerables presas.");
			...
			if (par_c.item2Id == "vagabunda") {
				if (par_c.item1Id == "anzuelo") {
					primitives.GD_CreateMsg ("es", "DLG_preguntar_a_vagabunda_por_anzuelo", "Le tengo mucho apego, me recuerda mi patria lejana.");
			...
			if ((par_c.item2Id == "vieja")||(par_c.item2Id == "hechicera")) {
				if (par_c.item1Id == "flautista") {
					primitives.GD_CreateMsg ("es", "DLG_preguntar_a_hechicera_por_flautista", "El flautista parece ser un simple campesino, pero es mucho más viejo y poderoso de lo que aparenta.");
					primitives.CA_QuoteBegin (par_c.item2Id, "DLG_preguntar_a_hechicera_por_flautista");
					return true;
				} else if (par_c.item1Id == "diamante") {
					primitives.GD_CreateMsg ("es", "DLG_preguntar_a_hechicera_por_diamante", "Es muy valioso para mí. Sin él esta cueva y toda mi vida es oscuridad.");
			...
			if (par_c.item2Id == "flautista") {
				if ((par_c.item1Id == "vieja")||(par_c.item1Id == "hechicera")) {
					...
					primitives.GD_CreateMsg ("es", "DLG_preguntar_a_flautista_por_hechicera", "La vieja de la cueva te maldijo por estar con quien ella cree que no debes. Pero quizás puedas hacerla entrar en razón si la satisfaces con algo.");
					...
					primitives.GD_CreateMsg ("es", "DLG_preguntar_a_flautista_por_comida", "Me da pereza ir a conseguir comida, prefiero que me la traigan.");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if (par_c.item1Id == "vieja") {
				...
				primitives.GD_CreateMsg ("es", "DLG_necesita_diamante", "Esta cueva está a oscuras desde que perdí mi piedra lunar. Antes esto resplandecía como un palacio. Te agradecería mucho si me lo recuperaras.");
				...
				if (primitives.IT_GetAttPropValue (primitives.PC_X(), "generalState", "state") == "4") { // no bebió la pócima
					if (primitives.IT_GetAttPropValue (primitives.IT_X("caldero"), "isLiquidContainer", "state") == "vacío") {
						primitives.GD_CreateMsg ("es", "DLG_pide_agua_púrpura", "Tráeme el agua púrpura para que te pueda romper el hechizo.");
						primitives.GD_CreateMsg ("es", "DLG_examina_pergamino", "Examina el pergamino para que sepas cómo obtenerla.");
						primitives.CA_QuoteContinues ("DLG_pide_agua_púrpura", [], false );
						primitives.CA_QuoteContinues ("DLG_examina_pergamino", [], false );
					} else {
						primitives.GD_CreateMsg ("es", "DLG_bebe_pócima", "Bebe la pócima del recuerdo, cuando consideres oportuno.");
						primitives.CA_QuoteContinues ("DLG_bebe_pócima", [], false );
					}

				} else {
					primitives.GD_CreateMsg ("es", "DLG_atraviesa_a_catarata", "Ahora ya lo sabes todo. Ve al otro lado de la catarata a buscar tu destino");
					primitives.CA_QuoteContinues ("DLG_atraviesa_a_catarata", [], false );
					if (primitives.PC_X() == primitives.IT_X("vagabunda")) {
						primitives.GD_CreateMsg ("es", "DLG_no_deberías_aquí", "Aunque no deberías hacerlo aquí.");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "bienvenida_juego", "Bienvenido a Las Tres Fuentes, aventura interactiva desarrollada en javascript. Sobre el mismo mundo base, puedes elegir entre jugar una versión reducida de tono más infantil o la versión completa, en la que controlas dos personajes.<br/>");
			primitives.GD_CreateMsg ("es", "chooseversion", "¿Qué versión eliges?");
			primitives.GD_CreateMsg ("es", "chooseversion_simple", "versión reducida");
			primitives.GD_CreateMsg ("es", "chooseversion_long", "versión completa");
			primitives.GD_CreateMsg ("es", "elegida_version_reducida", "Es una buena elección para acabarla en menos tiempo, pero la versión completa tiene un poco más de jugo.<br/><br/>");
			primitives.GD_CreateMsg ("es", "intro_reducida_1", "Eres Laila la dragona. La bruja se ha cansado de que revolotees por ahí con tu hermano Kuko quemando cosas a diestro y siniestro y os ha lanzado un hechizo.<br/>");
			primitives.GD_CreateMsg ("es", "intro_reducida_2", "Te ha convertido en humana y te ha sacado de la caverna del dragón, lejos de Kuko, al otro lado de la catarata.<br/>");
			primitives.GD_CreateMsg ("es", "intro_reducida_3", "No vas a quedarte con los brazos cruzados. Que se prepare la bruja, que se va a enterar. Kuko, no desfallezcas, ¡ya voy!<br/>");
			primitives.GD_CreateMsg ("es", "pulsa_para_ver_imagen_de_%o1", "Pulsa para ver imagen de %o1.<br/>");
			...
			if (currentLoc == primitives.IT_X("cruce_caminos")) {
				...
				if (primitives.IT_GetAttPropValue (currentLoc, "gameParameters", "version") == "") {
					...
					if (typeof par_c.option == 'undefined') { // phase 1: asking dialog
						...
						if (par_c.option == "simple") { // versión reducida
							...
							primitives.GD_CreateMsg ("es", "chooseversion_long_echo", "Has elegido la versión que elegiría un aventurero intrépido. Los protagonistas de este juego son dos amantes legendarios que han caído en desgracia ante una diosa opuesta a su amor. Aunque controlarás ambos personajes, elige ahora con quién empiezas, con la vagabunda, en realidad princesa del mar, que abandonó su patria por amor; o bien con el cazador, pariente de la diosa malvada.<br/><br/>");
				...
				if ((primitives.IT_GetAttPropValue (currentLoc, "gameParameters", "version") == "VC") && (gameParameters_FIRSTPC == "")) {
					...
					if (typeof par_c.option == 'undefined') { // phase 1: asking dialog
						...
						primitives.GD_CreateMsg ("es", "choosePC", "¿Con qué personaje quieres empezar jugando, ¿con la vagabunda o con el cazador?");
						primitives.GD_CreateMsg ("es", "chooseprimitives.PC_woman", "la vagabunda");
						primitives.GD_CreateMsg ("es", "chooseprimitives.PC_man", "el cazador");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "dejar_flauta", "Es un regalo del flautista, con quien te comprometiste a practicar antes de devolverla.<br/>");
			primitives.GD_CreateMsg ("es", "pájaros_vuelan", "Al soltar los pájaros se van volando a la fuente roja.<br/>");
			primitives.GD_CreateMsg ("es", "dejar_pájaros", "Al soltar los pájaros vuelven adonde estaban en la fuente roja.<br/>");
			...
			if (par_c.item1Id == "cuenco") {
				...
				if (primitives.IT_GetAttPropValue (primitives.IT_X("cuenco"), "isLiquidContainer", "state") == "pócima") {
					primitives.GD_CreateMsg ("es", "dejar_pócima", "¿Tirar algo que te ha costado tanto conseguir? Te lo piensas mejor y conservas la pócima.<br/>");
					primitives.CA_ShowMsg ("dejar_pócima");
					return true;
				}

				// cuenco se vacía al dejarlo
				primitives.GD_CreateMsg ("es", "dejar_cuenco", "El contenido del cuento se vacía al caer al suelo.<br/>");
				...
				if (primitives.IT_GetAttPropValue (primitives.IT_X("cuenco"), "isLiquidContainer", "state") == "dorada") {
					if (primitives.PC_GetCurrentLoc() == primitives.IT_X("tierras_sur")) {
						if (primitives.IT_GetAttPropValue (primitives.IT_X("tierras_sur"), "generalState", "state") == "0") {
							primitives.GD_CreateMsg ("es", "agua_dorada_mata_planta", "Vacias el agua sobre la planta, matándola y ensanchando un poco la entrada a la cueva.<br/>");
			...
			if ( ((par_c.pc == primitives.IT_X("cazador")) && (par_c.item1Id == "honda") && (primitives.IT_GetAttPropValue (primitives.IT_X("vagabunda"), "generalState", "state") == "0")) ||
				...
				primitives.GD_CreateMsg ("es", "dejar_arma_inicial", "Te puede ser útil para conseguir comida o para ganarte la confianza de alguien. Lo conservas.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "bebida_agotada","No hay mucho que beber.<br/>");
			primitives.GD_CreateMsg ("es", "beber_verde","Los filamentos verdes que tiene te dan algo de asco al principio, pero al final resulta que está rica y todo. En todo caso no te la bebes por completo.<br/>");
			primitives.GD_CreateMsg ("es", "beber_dorada","Apesta a azufre. Al acercar la nariz, instintívamente te echas atrás y no la bebes.<br/>");
			primitives.GD_CreateMsg ("es", "beber_roja","El rojo parece venir simplemente de restos de barro, sabe asquerosa y sólo la pruebas un poco, dejando el resto.<br/>");
			primitives.GD_CreateMsg ("es", "beber_naranja","Aún huele algo a azufre, pero el agua roja parece haberla hecho potable... pero no te atreves a probarla.<br/>");
			primitives.GD_CreateMsg ("es", "beber_púrpura","Un humano normal quizás no podría beberla, pero tú no eres un humano normal y este agua te trae recuerdos de la charca de la gruta. [PAUSA].  Al probarla te vienen recuerdos tristes de Kuko, ¿estará en la gruta?, ¿qué estará haciendo ahora?. No te la acabas, dejas para más tarde.<br/>");
			primitives.GD_CreateMsg ("es", "repetir_pócima","Ya habías bebido, apúrate y déjate de perder el tiempo.<br/>");
			primitives.GD_CreateMsg ("es", "aviso_pócima_grieta","Recuerda que la hechicera te dijo que si bebes recuperarás tu forma... y no podrías salir por la grieta.<br/>");
			primitives.GD_CreateMsg ("es", "empiezas_a_crecer_dentro_caverna", "A pesar de que sabes internamente que no deberías hacerlo, no puedes evitar beberte la pócima dentro de la cueva... empiezas a crecer...!<br/>");
			primitives.GD_CreateMsg ("es", "Menu_drinkNo_no_reaction", "Te lo piensas mejor y no te bebes la pócima todavía.<br/>");
			...
			if (liquido == "vacío") {
				...
				if ((primitives.IT_GetLoc(primitives.PC_X()) == primitives.IT_X("cueva_bruja")) && (primitives.PC_X() == primitives.IT_X("vagabunda"))) {
					...
					if (typeof par_c.option == 'undefined') { // fase 1: preguna
						...
						primitives.GD_CreateMsg ("es", "Menu_drinkNow", "¿Estás segura de que quieres beber ahora?");
						primitives.CA_ShowMsg ("Menu_drinkNow");
						primitives.GD_CreateMsg ("es", "Menu_drinkNo_yes", "Sí, quiero recuperar ya mi forma.");
						primitives.GD_CreateMsg ("es", "Menu_drinkNo_no", "No, esperaré a estar fuera.");
						...
						if (par_c.option == "drink_yes") { // bebes y no "deberías" poder salir
							...
							if (usr.getVersion() == "VR") {
								...
								primitives.GD_CreateMsg ("es", "VR_dragona_encerrada", "<br/><b>Teórico final del juego:</b> Al final la hechicera se salió con la suya y va a evitar que Kuko y tú hagáis más diabluras...<br/>");
								primitives.GD_CreateMsg ("es", "perdona_vida_dragona_encerrada", "<br/><b>Mensaje del escritor del juego:</b> Al crecer no deberías haber podido salir de la cueva... pero vamos a hacer un poco de trampas y te dejaremos seguir jugando, fuera de la cueva.<br/><br/>");
								primitives.GD_CreateMsg ("es", "deux_ex_machina_saprimitives.CA_dragona", "¡No! ¡No puedes dejar allí solito a Kuko sin nadie con quien jugar! Te precipitas a la grieta de salida y con tu nuevo cuerpo en crecimiento, agrandas la grieta y consigues salir de la cueva de la hechicera.<br/>");
								primitives.GD_CreateMsg ("es", "date_prisa", "¡Debes darte prisa para volver al otro lado de la cascada!<br/>");
				...
				primitives.GD_CreateMsg ("es", "cazador_recuerda_1", "Te acuerdas de cómo conociste a la hermosa princesa del mar cuando te hundiste en el océano en pos del anzuelo mágico que te había pedido tu hermano Hoderi.<br/>");
				primitives.GD_CreateMsg ("es", "cazador_recuerda_2", "Recuerdas el día que llegásteis a tierra firme, huyendo de la furia de su padre y os instalásteis en la cabaña de madera, vuestro hogar.<br/>");
				primitives.GD_CreateMsg ("es", "cazador_recuerda_3", "Recuerdas el día fatídico cuando tu pariente os condenó a olvidar que os habíais conocido.<br/>");
				primitives.GD_CreateMsg ("es", "cazador_recuerda_4", "De repende comprendes que está embarazada de ti y que necesitas estar con ellos, pero recuerdas el consejo de tu pariente lejana: - deja que la dragona de a luz a solas, o lo lamentarás el resto de tu vida.<br/>");
				primitives.GD_CreateMsg ("es", "cazador_recuerda_5", "Sabes que ella irá al otro lado de la catarata, ¿pero cuándo será el mejor momento para ir?<br/>");

				primitives.IT_SetAttPropValue (primitives.PC_X(), "generalState", "state", "5"); // bebió

				primitives.GD_CreateMsg ("es", "empiezas_recordar","Al beberte la pócima los recuerdos comienzan a aflorar con claridad.<br/>");
				...
				if (primitives.PC_X() == primitives.IT_X ("vagabunda")) {
					...
					primitives.GD_CreateMsg ("es", "VC_vagabunda2_recuerda_1", "Te acuerdas de cómo conociste al cazador, de naturaleza semidivina, nada menos que el un pariente lejano de la diosa de la montaña, cuando fue a tu mundo submarino en búsqueda de un anzuelo mágico para su hermano Hoderi<br/>");
					primitives.GD_CreateMsg ("es", "VC_vagabunda2_recuerda_2", "Recuerdas cómo os enamorásteis y huisteís porque porque las de tu especie deben dar a luz en tierra.<br/>");
					primitives.GD_CreateMsg ("es", "VC_vagabunda2_recuerda_3", "Recuerdas el día en que supiste que estabas embarazada y el cazador se lo contó a su pariente lejana, el día en que os maldijo usando su bastón de fuerza con el diamante mágico, en la cabaña del monte, vuestro hogar.<br/>");
					primitives.GD_CreateMsg ("es", "VC_vagabunda2_recuerda_4", "Y te acuerdas de tu naturaleza marina, que se empieza a derramar por entre tus piernas, has roto aguas y ya sabes lo que te ocurrirá, los de tu especie recuperan su horrenda naturaleza al alumbrar. Sabes que debes darte prisa y alumbrar a tu hijo en un lugar seguro, al otro lado de la cascada.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "no_acepto_regalos", "No acepto regalo de desconocidos.");
			primitives.GD_CreateMsg ("es", "da_gracias", "Pues gracias.");
			primitives.GD_CreateMsg ("es", "no_gracias", "No te preocupes, no me hace falta, gracias.");
			...
			if (par_c.item2 == usr.elOtroPJ(primitives.PC_X ())) {
				...
				if (primitives.IT_GetAttPropValue	(par_c.item2, "generalState", "state") == "0") {
					if ( ((par_c.item2Id == "cazador") && (par_c.item1Id == "anzuelo") ) ||
						...
						primitives.GD_CreateMsg ("es", "DLG_vagabunda_recibe_arma_1", "¡Muchas gracias, caballero!");
						primitives.GD_CreateMsg ("es", "DLG_cazador_recibe_arma_1", "¡Muchas gracias, guapa!");
						primitives.GD_CreateMsg ("es", "DLG_recibe_arma_2", "Creo que me puede ser útil. Hasta ahora le veía con desconfianza pero creo que me puedo fiar de ti. No sé por qué, pero me resultas familiar.");
			...
			// al flautista:
			primitives.GD_CreateMsg ("es", "flautista_hambriento_solo_quiere_comida", "Creo que no tienes nada de mi interés, lo que tengo es hambre.");
			if (par_c.item2Id == "flautista") {
				...
				primitives.GD_CreateMsg ("es", "flautista_rechaza_flauta_en_fuente_verde", "¿Crees que has practicado lo suficiente? Llévamela al risco lunar cuando estés preparado.");
				primitives.GD_CreateMsg ("es", "flautista_al_recibir_flauta_en_risco", "¿Crees que has practicado lo suficiente? Déjame oir qué tal tocas primero.");
				...
				if (par_c.item1Id == "diamante") { // recibe diamante
					primitives.GD_CreateMsg ("es", "gracias_por_traer_piedra", "¡Gracias por traerme la piedra lunar!");
					primitives.GD_CreateMsg ("es", "%o1 lo_guarda", "%o1 se lo guarda ávidamente.<br/>");

					primitives.CA_QuoteBegin (par_c.item2Id, "gracias_por_traer_piedra", [], false);
					primitives.CA_ShowMsg ("%o1 lo_guarda", {o1:"flautista"});
					primitives.IT_SetLoc (primitives.IT_X("diamante"), primitives.IT_X("flautista"));

					primitives.GD_CreateMsg ("es", "VC_pide_que_practiques", "Debéis honrar mis oidos con buena música si queréis que os ayude con lo de vuestro hechizo.");
					primitives.GD_CreateMsg ("es", "VR_pide_que_practiques", "Debes honrar mis oidos con buena música si quieres que te ayude con lo de tu hechizo.");
					...
					if (primitives.IT_GetLoc (primitives.IT_X("flautista")) == primitives.IT_X("risco_luna")) { // si ya en risco de la luna
						...
						primitives.GD_CreateMsg ("es", "te_espero_en_risco", "Te espero en el risco lunar.");
						primitives.GD_CreateMsg ("es", "ven_con_%o1", "No olvides venir con %o1.");
			...
			if (par_c.item1Id == "cuenco") {
				if (primitives.IT_GetAttPropValue (primitives.IT_X("cuenco"), "isLiquidContainer", "state") != "púrpura") {
					primitives.GD_CreateMsg ("es", "DLG_no_acepto_agua_no_púrpura", "¿Y para qué quiero yo esa vulgar agua?");
					primitives.CA_QuoteBegin (par_c.item2Id, "DLG_no_acepto_agua_no_púrpura");
				} else { // agua púrpura
					if (par_c.item2Id == "vieja") { // a la vieja
						primitives.GD_CreateMsg ("es", "DLG_dar_agua_púrpura_a_vieja", "Esa es una mezcla muy interesante. No sé cómo habrás averiguado para conseguirla, pero no me hace falta para nada.");
						primitives.CA_QuoteBegin (par_c.item2Id, "DLG_dar_agua_púrpura_a_vieja");
					} else if (par_c.item2Id == "hechicera") {
						if (primitives.IT_GetAttPropValue (primitives.IT_X("caldero"), "isLiquidContainer", "state") == "pócima")  {
							primitives.GD_CreateMsg ("es", "DLG_ya_me_lo_habías_dado", "Ya me la habías dado antes, no te esfuerces tanto.");
							primitives.CA_QuoteBegin (par_c.item2Id, "DLG_ya_me_lo_habías_dado");
						} else {
							if (usr.getVersion() == "VC") {
								if (primitives.IT_GetLoc(primitives.PC_X()) != primitives.IT_GetLoc(usr.elOtroPJ(primitives.PC_X ()))) {
									primitives.GD_CreateMsg ("es", "DLG_hechicera_pide_que_estén_ambos", "Debéis estar los dos aquí para que pueda hacer romper y fabricar la pócima de liberación.");
									primitives.CA_QuoteBegin (par_c.item2Id, "DLG_hechicera_pide_que_estén_ambos");
									return true;
								}
							}

							primitives.GD_CreateMsg ("es", "DLG_hechicera_recibe_agua_púrpura", "La hechicera casi te quita el cuenco de las manos y vierte su contenido en el caldero.");
							...
							primitives.GD_CreateMsg ("es", "hechicera_hace_pócima", "Al darle el agua púrpura, la hechicera te hace realiza un florido ritual al rededor del caldero. Rayos multicolor brillan del diamante, confluyendo en el caldero. El agua púrpura gira y gira, cambiando de color, hasta quedar de un color... eternamente cambiante.<br/>");
							primitives.CA_ShowMsg ("hechicera_hace_pócima");

							// aviso al dar la pócima
							if (usr.getVersion() == "VR") {
								primitives.GD_CreateMsg ("es", "DLG_VR_al_hacer_pócima_1", "Como recompensa por haberme traído antes el diamante, voy a devolverte tu aspecto original. Toma esta pócima y regresa a tu mundo, atravesando de nuevo la fuente verde");
								primitives.GD_CreateMsg ("es", "DLG_VR_al_hacer_pócima_2", "Pero recuerda: en cuanto bebas la pócima empezarás a crecer de manera incontenible y tendrás <b>poco tiempo</b> para atravesar la catarata y llegar a salvo en la gruta de los dragones, donde te espera tu amado Kuko. Si te retrasas, <u>volverás a tener tu forma humana</u> y quedarás separada de Kuko para siempre.");
								primitives.GD_CreateMsg ("es", "DLG_VR_al_hacer_pócima_3", "Adiós dragona, ¡buena suerte!");
								...
								primitives.GD_CreateMsg ("es", "DLG_VC_al_hacer_pócima_1", "Como recompensa por habedme traído antes el diamante, os voy a liberar de mi hechizo. Sigo siendo contraria a vuestra unión: no veo natural que una criatura del mar se una con un príncipe de las montañas. Pero sóis granditos y ya veréis lo que haréis.");
								primitives.GD_CreateMsg ("es", "DLG_VC_al_hacer_pócima_2", "Princesa Otohime, cuando tomes la pócima, empezarás a recuperar tu monstruosa forma original y en breve parirás a tu hijo. Ya casi no cabes por la grieta: no deberías beber aquí dentro. <b>¡Apresúrate!</b> Debes ir al otro lado de la catarata, lejos de la vista del cazador, que puede volverse loco al ver tu naturaleza sobrehumana.");
								primitives.GD_CreateMsg ("es", "DLG_VC_al_hacer_pócima_3", "Hoori, apártate de su camino y no acudas a ella hasta después del alumbramiento. Cuando llegue el momento lo sabrás.");
								primitives.GD_CreateMsg ("es", "DLG_VC_al_hacer_pócima_4", "Una última cosa. Después de que la princesa cruce la catarata, no podré evitar que se cumpla un pequeño efecto de mi maldición: la entrada a la catarata se cerrará y quedará sellada... ¡Pero qué mala soy!, ¡juá, juá, juá!");
								primitives.GD_CreateMsg ("es", "DLG_VC_al_hacer_pócima_5", "Adiós, estúpidos amantes, ¡vuestro amor está condenado!");
							...
							primitives.GD_CreateMsg ("es", "hechicera_hace_mutis", "El diamante brilla de manera cegadora y al recuperar la visión, ves que la hechicera ya no está, sólo el caldero con la pócima y el diamante, girando innacesible a gran altura e iluminando toda la cueva.<br/>");
							...
							// fin acto 3:
							primitives.GD_CreateMsg ("es", "fin_acto_3", "<br/><br/>Aquí termina el acto III: o de cómo la pócima fue obtenida y la hechicera hizo mutis por el foro.<br/><br/>");
							primitives.CA_ShowMsg ("fin_acto_3");
							primitives.PC_Points (25);

						}
					} else  { // cuenco a otros PNJ (flautista)
						primitives.GD_CreateMsg ("es", "DLG_no_quiero_eso", "¿Y para qué quiero esto? No gracias.");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if (par_c.item2Id == "flautista") {
				if (par_c.item1Id == "flauta") {
					...
					if (primitives.IT_GetAttPropValue (primitives.PC_X(), "generalState", "state") < "3") { // PJ no ha dado comida
						primitives.GD_CreateMsg ("es", "DLG_dame_comida_por_flauta", "Tengo mucha hambre, dame algo de comer y ya veré si te daré la flauta u otra cosa.");
						...
						primitives.GD_CreateMsg ("es", "DLG_esperando_comida_de_otro_por_flauta", "Estoy esperando a que otra persona se la gane.");
						primitives.GD_CreateMsg ("es", "DLG_flauta_ya_no_la_tengo", "Ya no la tengo.");
					...
					primitives.GD_CreateMsg ("es", "DLG_flautista_sobre_vieja_a_cazador", "Esa pariente tuya de la cueva, aunque no lo creas, te ha perjudicado en el pasado. Ten cuidado con ella.");
					primitives.GD_CreateMsg ("es", "DLG_flautista_sobre_vieja_a_vagabunda", "Esa vieja es una bruja y, aunque no lo creas, te ha perjudicado en el pasado. Ten cuidado con ella.");
			...
			primitives.GD_CreateMsg ("es", "DLG_vieja_sobre_diamante", "Lo perdí y no lo encuentro, ayúdame a encontrarlo, por favor.");
			primitives.GD_CreateMsg ("es", "DLG_vieja_sobre_flautista", "Le gusta mucho la música, pero es más que un simple flautista.");
			if (par_c.item2Id == "vieja")   {
			...
			if (par_c.item2Id == "hechicera")   {
				if (par_c.item1Id == "diamante") {
					primitives.GD_CreateMsg ("es", "DLG_hechicera_sobre_diamante", "Qué bueno que me lo conseguiste recuperar.");
					primitives.CA_QuoteBegin (par_c.item2Id, "DLG_hechicera_sobre_diamante");
					return true;
				} else if (par_c.item1Id == "flautista") {
					primitives.GD_CreateMsg ("es", "DLG_hechicera_sobre_flautista", "Ya veo que te dio la piedra de luz. Eres en verdad muy hábil.");
					primitives.CA_QuoteBegin (par_c.item2Id, "DLG_hechicera_sobre_flautista");
					return true;
				} else if (par_c.item1Id == "pócima") {
					primitives.GD_CreateMsg ("es", "DLG_hechicera_sobre_pócima", "Con el agua púrpura y la piedra lunar se puede obtener la pócima que deshaga mi anterior hechizo.");
...
let initItems =  function  (items, primitives) {
	...
	items.push ({
		...
		desc: function () {
			...
			primitives.GD_CreateMsg ("es", "VC_figura_roja", "La figura está formada por dos monstruos marinos de color rojo entrelazados que indican el camino del este. Bajo la figura se lee una inscripción con sus nombres: Toyotama-hime y Hoori.<br/>");
	...
	items.push ({
		...
		desc: function () {
			...
			primitives.GD_CreateMsg ("es", "fuente_roja_básico", "El agua aparenta ser venenosa y enfermiza");
			primitives.GD_CreateMsg ("es", "fuente_roja_con_pájaros %o1", ", pero increíblemente %o1 beben de ella de manera despreocupada<br/>");
	...
	items.push ({
		...
		desc: function () {
			...
			primitives.GD_CreateMsg ("es", "descripcion_fuente_verde_base", "¡La fuente verde! Una poderosa cascada cae desde una altura considerable y con gran fuerza. Un manto de musgo verde y líquenes crece por todos lados. Puedes ver un espectacular arcoiris causado por las gotitas de agua que saltan por todos lados tras chocar con el pequeño lago que se forma en la base, que también tiene una apariencia verde.<br/>");
			primitives.GD_CreateMsg ("es", "mencionar_flautista_con_flauta", "Ves a un joven campesino a la sombra de un árbol, tocando despreocupadamente la flauta.<br/>");
			primitives.GD_CreateMsg ("es", "mencionar_flautista_sin_flauta", "Ves al flautista sin su flauta, sentado despreocupadamente a la sombra de un árbol, cantando.<br/>");
	...
	items.push ({
		...
		desc: function () {
			...
			primitives.GD_CreateMsg ("es", "hay_basura", "No hay nada digno de mención, sólo matorrares y basura llevada por el viento.<br/>");
		...
		transitionTo: function (target) {
			...
			if (target == primitives.IT_X ("cueva_bruja")) {
				...
				primitives.GD_CreateMsg ("es", "pasas_malamente_1", "Consigues pasar por la grieta por los pelos.<br/>");
				primitives.GD_CreateMsg ("es", "pasas_malamente_2", "Consigues pasar por la grieta con un poquito de holgura.<br/>");
	...
	items.push ({
		...
		desc:	function () {
			...
			if (primitives.IT_GetAttPropValue (primitives.PC_X(), "generalState", "state") < "5") {
				primitives.GD_CreateMsg ("es", "cabaña_te_suena", "Los trastos quemados dispersos y arruinados te suenan de algo, pero no sabes muy bien de qué.<br/>");
				primitives.CA_ShowMsg ("cabaña_te_suena");
				return true;
			}

			// ya tienes memoria
			primitives.GD_CreateMsg ("es", "VC_vagabunda_recuerda_cabaña", "Ahora ves con otros ojos los restos de tu paraíso perdido, pero no tienes tiempo que perder regodeándote con el pasado.<br/>");
			primitives.GD_CreateMsg ("es", "VC_cazador_recuerda_cabaña", "Recuerdas cómo era todo antes, antes incluso de ir al mundo marino. ¡Qué hermosa era la cabaña que con tanto esfuerzo construiste!<br/>");
			primitives.GD_CreateMsg ("es", "VR_vagabunda_recuerda_cabaña", "No deberías perder el tiempo tan tontamente.<br/>");
	...
	items.push ({
		...
		desc:	function () {
			...
			primitives.GD_CreateMsg ("es", "en_cueva_oscura", "En la oscura cueva, en la casa de la vieja.<br/>");
			primitives.GD_CreateMsg ("es", "en_cueva_brillante", "En la resplandeciente cueva");
			primitives.GD_CreateMsg ("es", "hechicera_presente %o1", ", con %o1");
		...
		afterDescription: function () { // bienvenida de la vieja o de la hechicera
			...
			if (!primitives.IT_GetIsItemKnown (primitives.PC_X(), indexNPC)) { // aún era desconocida
				...
				if (idNPC == "vieja") {
					primitives.GD_CreateMsg ("es", "DLG_bienvenido_vagabunda", "Bienvenida a mi humilde hogar, princesa. Tú no me conoces, pero yo a ti sí. Sé que andas algo despistada. Si me encuentras la piedra lunar que perdí, te estaría muy agradecida.");
					primitives.GD_CreateMsg ("es", "DLG_bienvenido_cazador", "Bienvenido, querido pariente, a mi humilde hogar. No, no me mires con esa cara. Lo de tu cabaña fue un fatal incidente del que no quiero hablar. Sólo tienes que saber que te libré de una víbora que te tenía apresado.");
					primitives.GD_CreateMsg ("es", "DLG_bienvenido_cazador_2", "Verás que mi aspecto se ha deslucido desde la última vez que me viste. Ya no tengo la bella apariencia que tenía. El día de lo de tu cabaña perdí el diamante que conserva parte de mi poder, te agradecería si me lo pudieras recuperar.");
					...
					primitives.GD_CreateMsg ("es", "DLG_bienvenido_a_cueva_luminosa", "¿Te gusta cómo reluce mi cueva con el poder de la luz encerrada?");
	...
	items.push ({
		...
		talk: function (par_c) {
			...
			// here! primitives.GD_CreateMsg ("es", "", "<br/>");

			if (primitives.IT_GetAttPropValue (primitives.PC_X(), "generalState", "state") <= "1") {	// PJ aún no cazó
				primitives.GD_CreateMsg ("es", "que_hambre", "¡Qué hambre tengo! Te estaría muy agradecido si me dieras algo de comer.");
				...
				if (primitives.IT_GetLoc(primitives.IT_X(comida)) == primitives.PC_X()) { // si la lleva, encima, menú para facilitar donación
					...
					if (typeof par_c.option == 'undefined') { // preguntar
						...
						primitives.GD_CreateMsg ("es", "DLG_hambre_y_se_que_cazaste", "Hay que hambre tengo! Sé que has cazado algo, ");
						primitives.CA_QuoteBegin ("flautista", "DLG_hambre_y_se_que_cazaste", [], false);

						//  pedir comida y esperar si/no
						primitives.GD_CreateMsg ("es", "DLG_pregunta_si_das_comida", "¿Me das la comida?");
						primitives.CA_QuoteContinues ("DLG_pregunta_si_das_comida");

						primitives.GD_CreateMsg ("es", "flautista_das_comida_si", "Trato hecho!");
						primitives.GD_CreateMsg ("es", "flautista_das_comida_no", "Va a ser que no.");
						...
						if (par_c.option == "give_food_yes") { // acepta comida y da algo a cambio
							...
							primitives.GD_CreateMsg ("es", "DLG_qué_tacaño_eres", "qué tacaño eres, tú te lo pierdes. Yo sigo con lo mío, que cuando toco la música se me olvida el hambre.");
							primitives.CA_QuoteContinues ("DLG_qué_tacaño_eres");

							primitives.GD_CreateMsg ("es", "flautista_toca", "El flautista toca su tonada...");
							primitives.CA_PlayAudio ("flauta1.mp3", true, "flautista_toca");
						}
					}
				} else {
					primitives.GD_CreateMsg ("es", "DLG_ven_con_comida", "vuelve con lo que cazaste y hablamos.");
				...
				if (primitives.PC_GetCurrentLoc() == primitives.IT_X("risco_luna")) {
					...
					if (typeof par_c.option == 'undefined') { // preguntar
						primitives.GD_CreateMsg ("es", "DLG_espero_músiprimitives.CA_con_diamante", "Espero escuchar buena música bajo la magia de la luna.");
						...
						if ((primitives.PC_X() == primitives.IT_X("vagabunda")) && (primitives.IT_GetLoc(primitives.IT_X("diamante")) ==  primitives.IT_X("limbo"))) {
							primitives.GD_CreateMsg ("es", "DLG_recuerda_diamante", "Recuerda que con tu visión de serpiente en la cabaña algo debes encontrar.", [], false);
							primitives.CA_QuoteContinues ("DLG_recuerda_diamante");
						}
						primitives.CA_QuoteContinues ("");
					}
				} else {
					primitives.GD_CreateMsg ("es", "DLG_finito_te_veo_en_risco", "Ya te di lo que necesitabas. Cuando estés preparado, ve al risco de la luna.");
				...
				if (primitives.IT_GetLoc (primitives.IT_X("flauta")) == primitives.PC_X() ) { // llevas la flauta, el flautista está esperando a que toques la flauta
					...
					if (typeof par_c.option == 'undefined') { // preguntar
						primitives.GD_CreateMsg ("es", "DLG_pregunta_si_tocas", "Venga, dame un gusto, y tócame una buena tonada bajo el influjo de la luna.");
						primitives.CA_QuoteBegin ("flautista", "DLG_pregunta_si_tocas");

						primitives.GD_CreateMsg ("es", "tocas_flauta_si", "Vale, voy a probar, pero no creo que sepa.");
						primitives.GD_CreateMsg ("es", "tocas_flauta_no", "Lo siento, pero no sé tocar la flauta.");
						...
						if (par_c.option == "play_flute_yes") {
							...
							primitives.GD_CreateMsg ("es", "DLG_flautista_sin_oirte", "Así nunca sabré si has aprendido lo suficiente.");
				...
				primitives.GD_CreateMsg ("es", "DLG_envía_saludos_a_vieja", "Dale saludos a la vieja de mi parte.");
				primitives.CA_QuoteBegin ("flautista", "DLG_envía_saludos_a_vieja");
				return true;
			} else { // ya bebió
				primitives.GD_CreateMsg ("es", "DLG_ya_bebiste_apurate", "Ya bebiste la pócima, apúrate con lo que debas hacer.");
	...
	if (indexItem == primitives.PC_X()) { // reacción sobre PJ, independiente de la acción elegida
		...
		if (usr.getVersion() == "VC")  {
			...
			if (primitives.PC_GetTurn() == 300) {
				if (primitives.PC_X() == primitives.IT_X("vagabunda") ) {
					primitives.GD_CreateMsg ("es", "vagabunda_no_aguanta_más", "<br/>¡No aguantas más! El alumbramiento será ahora mismo.<br/>");
			...
			if (primitives.PC_GetTurn() % 15 == 0) {
				...
				if (primitives.PC_X() == primitives.IT_X("vagabunda")) {
					primitives.GD_CreateMsg ("es", "contracción %s1", "<br/><br/>¡Qué dolor! Te detienes en seco por una contracción.De alguna forma sabes que te quedan %s1 contracciones antes de dar a luz.<br/>");

					primitives.CA_ShowMsg ("contracción %s1" , {s1:contraciones_quedan});
				} else if (primitives.IT_GetLoc(primitives.PC_X()) == primitives.IT_GetLoc(usr.elOtroPJ (primitives.PC_X()))) { // misma loc
					primitives.GD_CreateMsg ("es", "contracción_vista_por_cazador %s1", "<br/><br/>Ves cómo la vagabunda se detiene y se retuerce de dolor. De alguna forma sabes que le quedan %s1 contracciones antes de dar a luz.<br/>");
		...
		if (primitives.IT_GetAttPropValue (primitives.IT_X("pócima"), "hasDecrementor", "active") == "true" ) {
			...
			if (primitives.PC_X() == primitives.IT_X("vagabunda")) {
				...
				if (counter < 4) {
					...
					primitives.GD_CreateMsg ("es", "cada_vez_más_grande", "Cada vez estás más grande!!<br/>");
					primitives.CA_ShowMsg("cada_vez_más_grande");
					if (counter < 0) {

						if (usr.getVersion() == "VR") {
							primitives.GD_CreateMsg ("es", "perdona_vida_conversión_externa", "<br/><b>Mensaje del escritor del juego:</b> Te volviste a hacer humana y ya no podrás atravesar la catarata y reunirte con Kuko... pero te perdonamos la vida (no se lo digas a nadie) y te dejamos seguir jugando como dragona...<br/><br/>");
							primitives.GD_CreateMsg ("es", "deux_ex_machina_dragona_vuela", "Cuando completas tu transformación, desplegas tus alas y atraviesas volando la catarata.<br/>");
			...
			if (primitives.IT_GetLoc (indexItem) == primitives.PC_GetCurrentLoc()) { // misma localidad
				...
				if (primitives.IT_GetAttPropValue (indexItem, "generalState", "state") == "0") {
					...
					if (primitives.IT_GetAttPropValue (indexItem, "generalState", "confianza") == "0") {
						...
						primitives.GD_CreateMsg ("es", "%o1_desconfia", "%o1 te mira con desconfianza, no en balde sóis unos desconocidos.");
						...
						primitives.GD_CreateMsg ("es", "DLG_me_voy", "Disculpe que me vaya, no frecuento a desconocidos.");
...
usr.getVersion = function () {
	...
	return primitives.IT_GetAttPropValue (primitives.IT_X("cruce_caminos"), "gameParameters", "version");
}
...
usr.examenMusical = function (par_c) {
	...
	primitives.GD_CreateMsg ("es", "DLG_anima_música", "¡Venga, dale!");
	...
	primitives.GD_CreateMsg ("es", "DLG_tocas_nivel_0", "Socorro, qué dolor! Vete de mi vista y no vuelvas hasta que hayas practicado más.");
	primitives.GD_CreateMsg ("es", "DLG_tocas_nivel_1", "¿Cómo te atreves a hacer ese ruido sin haber practicado casi nada?");
	primitives.GD_CreateMsg ("es", "DLG_tocas_nivel_2", "Vas mejorando, pero aún debes practicar algo más.");
	primitives.GD_CreateMsg ("es", "DLG_tocas_nivel_3", "Practica un poco más y lo tendrás controlado.");
	primitives.GD_CreateMsg ("es", "DLG_tocas_nivel_4", "¡Enhorabuena, estás hecho un virtuoso! Ahora ya eres digno de que yo toque la flauta para ti. Honraremos a la luna y ella hará algo por ti.");
	...
	if (primitives.IT_GetAttPropValue (primitives.PC_X(), "generalState", "nivelMusical") == "0") {
		...
		if ( (primitives.IT_GetLoc(primitives.IT_X("diamante")) != primitives.PC_X()) && (primitives.IT_GetLoc(primitives.IT_X("diamante")) != primitives.IT_X("flautista")) ) { // no aquí
			primitives.GD_CreateMsg ("es", "DLG_debes_traerme_diamante", "Pero para ello, la piedra lunar debe estar presente.");
		...
		if (primitives.IT_GetLoc(primitives.IT_X("diamante")) == primitives.PC_X()) {
			primitives.GD_CreateMsg ("es", "flautista_recupera_diamante", "El flautista te quita el diamante, como quien recupera un viejo tesoro.");
...
usr.escenaBaile = function() {
	...
	primitives.GD_CreateMsg ("es", "VC_transformación_flautista_1", "El diamante brilla con una irreal hermosura. Vosotros, aún siendo casi unos desconocidos, os tomáis de la mano y comenzáis a bailar, horas y horas.<br/>");
	primitives.GD_CreateMsg ("es", "VC_transformación_flautista_2", "Algunos recuerdos reaparecen, recordáis haber bailado entre corales y medusas. Cuando parece que el velo del olvido os abandona... cesa la música y con ella esos dulces recuerdos.<br/>");
	primitives.GD_CreateMsg ("es", "VC_transformación_flautista_3", "Tardáis un momento en daros cuenta de que ha amanecido y el flautista ha dejado de tocar y ha vuelvo a recuperar la forma del flautista campestre.<br/>");
	primitives.GD_CreateMsg ("es", "VR_transformación_flautista_1", "El diamante brilla con una irreal hermosura. Presa de una alegría incontrolada, comienzas a dar vueltas y bailar, horas y horas, hasta que cesa la música y tardas un momento en darte cuenta de que ha amanecido y el elfo ha dejado de tocar y ha vuelvo a recuperar la forma del flautista campestre.<br/>");
	primitives.GD_CreateMsg ("es", "VR_transformación_flautista_2", "");
	primitives.GD_CreateMsg ("es", "VR_transformación_flautista_3", "");
	primitives.GD_CreateMsg ("es", "DLG_VC_id_con_vieja", "Id adonde la vieja bruja, ella sabrá perdonaros el hechizo con que os maldijo si le lleváis el diamante.");
	primitives.GD_CreateMsg ("es", "DLG_VR_id_con_vieja", "Vete adonde la vieja bruja, ella sabrá perdonarte el hechizo con que os maldijo a ti y a Kuko, si le llevas el diamante.");
	primitives.GD_CreateMsg ("es", "flaustista_desaparece_tras_árbol", "El flautista da un brinco y desaparece sin más detrás de un árbol. Al seguir sus huellas, ves que desaparecen sin más detrás del árbol.<br/>");
	primitives.GD_CreateMsg ("es", "VC_fin_acto_2", "<br/><br/>Aquí termina el acto II: o de cómo el flautista hizo bailar a los dos tortolitos.<br/><br/>");
	primitives.GD_CreateMsg ("es", "VR_fin_acto_2", "<br/><br/>Aquí termina el acto II: o de cómo el flautista hizo bailar a Laila hasta el amanecer.<br/><br/>");

	if (usr.getVersion() == "VC") {
		// marcamos al otro PJ que también bailó
		primitives.IT_SetAttPropValue (usr.elOtroPJ(primitives.PC_X ()), "generalState", "state", "4");
	}

	primitives.GD_CreateMsg ("es", "DLG_VC_toma_diamante", "Tomad el diamante que tantos ansiáis. Id con él a la vieja bruja de la cueva.");
	primitives.GD_CreateMsg ("es", "DLG_VR_toma_diamante", "Toma el diamante que tanto ansías. Ve con él a la vieja bruja de la cueva.");
	primitives.GD_CreateMsg ("es", "DLG_VC_busqué_pero_no_encontré_diamante", "El día del maleficio yo vi por una ventana cómo la bruja os maldijo usando la piedra de luz. Vi que después de un rayo terrible, la piedra salió despedida... pero nunca la llegué a encontrar por mucho que busqué entre los restos del incendio.");
	primitives.GD_CreateMsg ("es", "DLG_VR_busqué_pero_no_encontré_diamante", "Por la cabaña, esa que quemaste por accidente jugando con Kuko, vi cómo la bruja te lanzó un hechizo, usando la piedra de luz y te transformó en humana, encerrando a Kuko al otro lado de la catarata. Después de un rayo terrible, la piedra salió despedida... pero nunca la llegué a encontrar.");
	primitives.GD_CreateMsg ("es", "DLG_es_hora_de_bailar", "Pero olvidemos el pasado... ¡hora es tiempo de bailar!");

	primitives.CA_QuoteBegin ("flautista", "DLG_" + usr.getVersion() + "_toma_diamante", [], false);
	primitives.CA_QuoteContinues ("DLG_" + usr.getVersion() + "_busqué_pero_no_encontré_diamante", [], false);
	primitives.CA_QuoteContinues ("DLG_es_hora_de_bailar");

	// escena del baile
	primitives.CA_ShowMsg (usr.getVersion() + "_transformación_flautista_1");
	primitives.CA_ShowMsg (usr.getVersion() + "_transformación_flautista_2");
	primitives.CA_ShowMsg (usr.getVersion() + "_transformación_flautista_3");

	primitives.CA_QuoteBegin ("flautista", "DLG_" + usr.getVersion() + "_id_con_vieja");

	primitives.CA_ShowMsg ("flaustista_desaparece_tras_árbol");

	// fin acto 2:
	primitives.CA_ShowMsg (usr.getVersion() + "_fin_acto_2");
	primitives.PC_Points (25);
...
usr.flautistaRecibeComida = function(comida) {
	...
	primitives.GD_CreateMsg ("es", "DLG_gracias_por_o1", "¡Gracias por %o1! Los prepararé a la brasa.");
	primitives.CA_QuoteBegin ("flautista","DLG_gracias_por_o1",[comida], false);

	if (regalo == "flauta") {
		primitives.GD_CreateMsg ("es", "DLG_toma_flauta", "Toma la flauta, practica con ella. Cuando hayas alcanzado suficiente maestría con ella, búscame y demuéstrame lo que sabes si quieres mi ayuda.");
	...
	if ((regalo != "flauta") || (usr.getVersion() == "VR") ) {
		primitives.GD_CreateMsg ("es", "DLG_aviso_espejo", "Debes saber que tu visión de serpiente te permite ver lo que a otros está oculto. Mira con calma <u>en la cabaña</u> y tráeme lo que encuentres.");
	...
	primitives.GD_CreateMsg ("es", "DLG_me_voy_al_risco", "Creo que me iré a echar una siesta al risco de la luna.");
	primitives.GD_CreateMsg ("es", "DLG_te_espero_vagabunda", "Te espero allí, cuando estés list con mi piedra, princesa. A medianoche, con el cazador.");
	primitives.GD_CreateMsg ("es", "DLG_te_espero_cazador", "Te espero allí, cuando estés listo para regalarme una bella tonada, cazador. A medianoche, con la vagabunda.");
	primitives.GD_CreateMsg ("es", "DLG_VC_adelanto_baile", "Si me complacéis, esta noche la piedra lunar resplandecerá de manera especial y recuperaréis algo que habíais perdido, aunque sólo sea por un rato.");
	primitives.GD_CreateMsg ("es", "DLG_VR_adelanto_baile", "Te espero allí, cuando estés lista. A medianoche ven a visitarme, pero sólo cuando hayas alcanzado maestría con la flauta y tengas la piedra lunar, que resplandecerá esta noche de manera especial.");
	primitives.GD_CreateMsg ("es", "DLG_aún_espero_comida_del_otro", "Espero que tu media naranja me traiga algo de comer también.");

	primitives.CA_QuoteContinues ("DLG_me_voy_al_risco", [], false);
	if ((usr.getVersion() == "VC") && (primitives.IT_GetAttPropValue (usr.elOtroPJ(primitives.PC_X()), "generalState", "state") == "3")) {
		primitives.CA_QuoteContinues ("DLG_te_espero_"+ primitives.IT_GetId(primitives.PC_X()), [], false);
		primitives.IT_SetLoc (primitives.IT_X("flautista"), primitives.IT_X("risco_luna"));
		primitives.CA_QuoteContinues ("DLG_" + usr.getVersion() + "_adelanto_baile", [], false);
	...
	if ( (usr.getVersion() == "VR") ||
		 ( (usr.getVersion() == "VC") &&
		   (primitives.IT_GetAttPropValue (primitives.PC_X(), "generalState", "state") == "3") &&
		   (primitives.IT_GetAttPropValue (usr.elOtroPJ (primitives.PC_X()), "generalState", "state") == "3") ) ) {
		// fin acto 1
		primitives.GD_CreateMsg ("es", "VC_fin_acto_1", "<br/><br/>Aquí termina el acto I: o de cómo los dos amantes desmemoriados se reencontraron y colaboraron para dar de comer al hambriento flautista, que les dio una flauta y un consejo.<br/><br/>");
		primitives.GD_CreateMsg ("es", "VR_fin_acto_1", "<br/><br/>Aquí termina el acto I: o de cómo Laila dio de comer al hambriento flautista y a cambio recibió una flauta para practicar música.<br/><br/>");
		primitives.PC_Points (25);
		primitives.CA_ShowMsg (usr.getVersion() + "_fin_acto_1");
	}
...
usr.talkPJ = function (par_c) {
	...
	if (!primitives.IT_GetIsItemKnown (par_c.pc, par_c.item1)) {
		primitives.GD_CreateMsg ("es", "DLG_primer_saludo", "Buenos días, ¿nos conocemos? Su cara me resulta familiar.");
	...
	*/
	...
	if ((EPJ<4) && (EPNJ<4)) {
		if (EPNJ == "0") { // PNJ no confía aún en PJ
			primitives.GD_CreateMsg ("es", "DLG_No_le_conozco", "No le conozco, creo que no debería hablar con usted.");
			primitives.CA_QuoteContinues ("DLG_No_le_conozco", [], false );
		} else if (EPNJ < EPJ) {// PJ más avanzado que el PNJ
			primitives.GD_CreateMsg ("es", "DLG_deberías_ayudar_a_%o1", "No tiene nada que aportarte a ti... pero tú sí a %o1.");
			...
			primitives.GD_CreateMsg ("es", "DLG_yo_confío", "Yo confío en ti, ¿qué podría hacer para que confiaras en mí?");
			primitives.CA_QuoteContinues ("DLG_yo_confío", [], false );
		} else if (EPNJ == EPJ &&  EPJ == "1" ) { // ya intercambiaron armas
			primitives.GD_CreateMsg ("es", "DLG_deberíamos_usar_armas", "Deberíamos usar las armas que hemos intercambiado.");
			primitives.CA_QuoteContinues ("DLG_deberíamos_usar_armas", [], false );
		} else if (EPNJ >= "2" && EPJ == "1" ) { // PNJ cazó, pero PJ aún no
			primitives.GD_CreateMsg ("es", "DLG_deberías_cazar", "Yo ya me cobré mi pieza, deberías hacer lo propio.");
			primitives.CA_QuoteContinues ("DLG_deberías_cazar", [], false );
		} else if (EPNJ == EPJ && EPJ == "2"  ) { //ambos cazaron
			primitives.GD_CreateMsg ("es", "DLG_deberías_visitar_flautista_1", "quizás habría que ir a visitar al flautista de nuevo.");
			...
			primitives.GD_CreateMsg ("es", "DLG_deberías_visitar_flautista_2", "creo que el flautista podría ayudarte.");
			primitives.CA_QuoteContinues ("DLG_deberías_visitar_flautista_2", [], false );
		} else if (EPNJ == EPJ && EPJ == "3" ) {
			primitives.GD_CreateMsg ("es", "DLG_recordar_flautista_en_risco", "El flautista nos citó en el risco de la luna con la flauta y la piedra lunar.");
			primitives.CA_QuoteContinues ("DLG_recordar_flautista_en_risco", [], false );
		} else {
			primitives.GD_CreateMsg ("es", "DLG_no_sé_qué_decir", "No sé qué decirte.");
	...
	primitives.GD_CreateMsg ("es", "DLG_debemos_conseguir_pócima", "¡Tenemos que conseguir la pócima!");
	primitives.GD_CreateMsg ("es", "DLG_debemos_llevar_agua_púrpura", "Deberíamos llevar el agua púrpura para llevársela a la hechicera.");
	primitives.GD_CreateMsg ("es", "DLG_vagabunda_aviso_1", "Por favor, aléjate del camino que lleva a la catarata, no te gustará verme después de que tome la pócima, hasta después del parto.");
	primitives.GD_CreateMsg ("es", "DLG_cazador_aviso_1", "Estoy muy nervioso, no sé qué ocurrirá cuando tomes la pócima; tengo miedo, no quiero dejarte sola.");
	...
	primitives.GD_CreateMsg ("es", "DLG_cazador_aviso_2", "Ya lo recuerdo todo, cariño. Recuerda que antes de beber tú, deberías dejar que me esconda.");
	if ((primitives.PC_X() == primitives.IT_X("vagabunda")) && (EPJ == 4)) { // vagabunda habla con cazador (quien ha bebido pócima)
		...
		primitives.GD_CreateMsg ("es", "DLG_te_avisé", "¡Te había avisado!, ¡aparta de mi camino!");
		...
		primitives.GD_CreateMsg ("es", "DLG_vagabunda_aviso_2", "Cariño, ya lo recuerdas todo, pero yo cuando tome la pócima me transformaré en algo que no quiero que veas: aparta de mi camino a la catarata o lo lamentarás de la peor de las maneras.");
		...
		// ya habría acabado el juego, al ver visto a la dragona
		primitives.GD_CreateMsg ("es", "DLG_no_puede_ser", "¡No puede ser!, ¡monstruo del infierno!");
...
usr.comprobarSiFinDelJuego = function() {
	...
	if (usr.getVersion() == "VR") {
		...
		primitives.GD_CreateMsg ("es", "VR_ganaste", "Atraviesas la cascada verde y entras en tu cueva, tu hogar. Después de avanzar por un túnel llegas a una inmensa cueva repleta de monedas de oro y diamantes. Sobre ella reposa Kuko el dragón, que al verte lanza un fuego de alegría al aire y vuela hacia ti y te da un gran abrazo dragoniano con sus escamosas alas. Has vuelto al hogar, ¡¡HAS GANADO!!<br/>");
		primitives.CA_ShowMsg ("VR_ganaste");

		primitives.CA_ShowImg ("dos_dragones.jpg", true);

		// fin acto 4 (ganando)
		primitives.GD_CreateMsg ("es", "VR_fin_acto_4", "<br/><br/>Aquí termina el acto IV y último: o de cómo los dragones se reunieron y fueron buenos... por un tiempo.<br/><br/>");
		...
		if (primitives.PC_X() == primitives.IT_X("vagabunda")) {
			...
			primitives.GD_CreateMsg ("es", "dragona_en_cueva", "Atraviesas la cascada verde y entras en tu cueva. Tras de ti, como profetizó la hechicera, rocas enormes caen y la catarata queda inaccesible desde el exterior.<br/>");
			...
			if (primitives.IT_GetLoc (primitives.IT_X("cazador")) == primitives.IT_X("caverna_dragón")) {
				...
				primitives.GD_CreateMsg ("es", "dragona_en_cueva_pero_cazador_presente", "Al entrar en el que debería haber sido tu refugio... tu dicha se torna drama. El cazador está presente y te ve llegar.<br/>");
				...
				primitives.GD_CreateMsg ("es", "sale_bebé_del_huevo", "Mientras te recuperas, observas cómo unas manitas rompen el huevo desde dentro. El huevo se desmorona dejando a la vista un bebé que rompe a llorar.<br/>");
				...
				if (primitives.IT_GetLoc (primitives.IT_X("cazador")) == primitives.IT_X("escondite")) { // cazador la siente llegar... pero no la ve siendo dragona
					...
					primitives.GD_CreateMsg ("es", "cazador_coge_bebé", "Una vez has recuperado tu forma humana, el cazador aparece de entre las sombras. Te abriga con una manta y coge al bebé en sus brazos, observando que, como su madre, tiene ojos de serpiente. Sois una extraña familia, pero una familia al fin y al cabo. Vuestro hijo, al que llamáis Ugayafukiaezu, está destinado a ser el padre del primer emperador del Japón.<br/>");
					primitives.CA_ShowMsg ("cazador_coge_bebé");

					primitives.GD_CreateMsg ("es", "VC_fin_acto_4", "<br/><br/>Aquí termina el acto IV y último: o de cómo la feliz familia se reunió y el maleficio superó.<br/><br/>");
					...
					primitives.primitives.GD_CreateMsg  (1, "dragona_recupera_forma_encerrada", "Recuperas tu forma y quedas con tu hijo en brazos. No hay salida posible para tu hijo. Por aquí corre un río subterráneo. Con lágrimas en los ojos, lo abandonas y sigues el curso del río, hacia tu patria subterránea.<br/>");
					primitives.CA_ShowMsg ("dragona_recupera_forma_encerrada");

					primitives.GD_CreateMsg ("es", "VC_fin_acto_4_niño_a_solas", "<br/><br/>Aquí termina el acto IV y último: o de cómo el maleficio no fue superado y el niño-dragón, quedó abandonado.<br/><br/>");
...
usr.epilogoAbandono = function() {
	...
	primitives.GD_CreateMsg ("es", "VC_fin_acto_4_niño_a_solas_epílogo", "<br/><br/><b>Epílogo:</b> Una vez que la princesa del mar abandona a su vástago, aparece la hechicera.<br/><br/>");
	primitives.CA_ShowMsg ("VC_fin_acto_4_niño_a_solas_epílogo");

	primitives.GD_CreateMsg ("es", "DLG_no_tienes_culpa_dragoncito", "Tú no tienes culpa de nada, niño dragón. Te llamaré Ugayafukiaezu. Con tu naturaleza y mis poderes serás poderoso.");
...
usr.epilogoCazadorAusente = function() {
	...
	primitives.GD_CreateMsg ("es", "VC_fin_acto_4_cazador_ausente", "<br/><br/><b>Epílogo:</b> Una vez que el cazador huye desconosolado del lugar, aparece la hechicera.<br/><br/>");
...
usr.creditos = function() {
	...
	primitives.GD_CreateMsg ("es", "gracias_por_jugar", "Gracias por jugar. Si no jugaste a la otra versión, te animamos a que lo hagas y veas las diferencias, se trata de una historia ligeramente diferente.<br/>");
	primitives.CA_ShowMsg ("gracias_por_jugar");
	primitives.GD_CreateMsg ("es", "disclaimer", "Disculpa los bugs y fallos con los que probablemente te has encontrado en el juego. No dudes en hacernos llegar tus comentarios a %uludi.ludon@gmail.com__mailto:ludi.ludon@gmail.com%<br/>");
	primitives.CA_ShowMsg ("disclaimer");
	primitives.GD_CreateMsg ("es", "próximas_versiones", "En próximas versiones del motor del juego, mejoraremos la jugabilidad y la parte colaborativa en red.<br/>");
	primitives.CA_ShowMsg ("próximas_versiones");

	primitives.GD_CreateMsg ("es", "making_of_0", "<br/><b>The making of Las Tres Fuentes</b><br/><br/>");
	primitives.CA_ShowMsg ("making_of_0");

	primitives.GD_CreateMsg ("es", "making_of_1", "La idea inicial del juego partió de la cascada de colores, en la isla de La Palma, islas Canarias, en el Parque Nacional de La Caldera de Taburiente. Si puedes ir alguna vez, disfrutarás mucho tanto de la cascada como de la caminata hasta llegar a ella.<br/>");
	primitives.CA_ShowMsg ("making_of_1");

	if (usr.getVersion() == "VC") {
		primitives.GD_CreateMsg ("es", "making_of_2", "La idea del embarazo de la criatura marina, procede de la mitología japonesa: tira del hilo en %uhttps://es.wikipedia.org/wiki/Toyotama-hime__https://es.wikipedia.org/wiki/Toyotama-hime%, o buscando por Toyotama-hime (vagabunda), Hoori (cazador), Ōyamatsumi (bruja, diosa de las montañas) y Ugayafukiaezu (el niño dragón), que supuestamente sería el padre del emperador Jinmu, legendario primer emperador del Japón.<br/>");
...
usr.perdiste_transformacionExterna = function() {
	...
	var cazadorPresente = (primitives.IT_GetLoc(primitives.PC_X()) == primitives.IT_GetLoc(usr.elOtroPJ(primitives.PC_X ())));

	primitives.GD_CreateMsg ("es", "locura_3a_persona", "El cazador te observa espantado. Te ves reflejada en sus ojos, a medio transformar en tu forma marina. Ves cómo tu antes amado compañero se mea y caga encima y se va corriendo, espantado.<br/>");
	if (cazadorPresente) primitives.CA_ShowMsg ("locura_3a_persona");

	primitives.GD_CreateMsg ("es", "transformación_1a_persona", "Concluyes tu transformación. Tu inmensa figura escamosa que algunos llaman dragón y otros, en lejanas tierras, sirena, no te deja moverte con facilidad. No es esta la forma en la que te conoció el cazador. Aquella era una mexcla grácil de ambos mundos; ahora tienes la forma puramente marina, sin rastro de humanidad.<br/>");
	primitives.GD_CreateMsg ("es", "alumbramiento_1a_persona", "Con un ligero esfuerzo, de tu cuerpo sale un gran huevo rodeado de viscosidades varias.<br/>");
	primitives.GD_CreateMsg ("es", "hechicera_expulsa_dragona_según_vagabunda", "Al rato, aparece la hechicera y con un sortilegio, esta vez inquebrantable, te envía de vuelta a tu mundo submarino. Nunca sabrás si la naturaleza de tu hijo será humana o marina, ni lo verás crecer, ni sabrás qué será de tu amante cazador.<br/>");
	primitives.GD_CreateMsg ("es", "quien_sabe_si", "Quién sabe si en otro mundo paralelo el final de esta historia podría haber sido más feliz.<br/>");
	...
	// fin acto 4 (perdiendo)
	primitives.GD_CreateMsg ("es", "fin_acto_4_conversión_externa_sin_cazador", "<br/><br/>Aquí termina el acto IV y último: o de cómo la dragona parió y a su hijo abandonó.<br/><br/>");
	primitives.GD_CreateMsg ("es", "fin_acto_4_conversión_externa_con_cazador", "<br/><br/>Aquí termina el acto IV y último: o de cómo el cazador enloqueció, la dragona parió y a su hijo abandonó.<br/><br/>");
	if (cazadorPresente)
...
usr.aOtraLocalidad = function(indexItem) {
	...
	primitives.GD_CreateMsg ("es", "o1_se_va_a_o2", "<br/>%o1 se va a %o2.<br/>");
	primitives.CA_ShowMsg("o1_se_va_a_o2", {o1:primitives.IT_GetId(indexItem), o2:nuevaLoc} );
...
usr.intro = function() {
	...
	if (primitives.IT_GetAttPropValue (primitives.IT_X("cruce_caminos"), "gameParameters", "firstPC") == "vagabunda") {
		...
		primitives.GD_CreateMsg ("es", "introWoman_1", "Eres la princesa del mar, deberías estar nadando con otras criaturas como tú... en vez de estar como ahora, con dos piernas y en tierra firme.");
		primitives.GD_CreateMsg ("es", "introWoman_2", "Imágenes confusas. Un guapo humano en tu mundo... luego, ya en tierra, una cabaña de madera, un incendio. Ruido, dolor... y ahora aquí, en mitad de ninguna parte.");
		primitives.GD_CreateMsg ("es", "introWoman_3", "Te levantas y te notas pesada. No sólo estás presa en este cuerpo humano y fuera de tu elemento, sino que descubres que estás embarazada y no sabes ni quién es el padre.<br/>");
		primitives.GD_CreateMsg ("es", "introWoman_4", "-¡Padre!, ¡madre!, ¡quiero volver con vosotros!- gritas desconsoladamente.");
		primitives.GD_CreateMsg ("es", "introWoman_5", "Sin emabargo, sabes que las de tu raza cuando son concebidas por humanos deben dar a luz en tierra. Tu hijo podrá nacer humano u... otra cosa, más afín a ti, pero no lo sabrás hasta que llegue ese momento, muy cercano.<br/>");
		primitives.GD_CreateMsg ("es", "introWoman_6", "¡Ui! Notas una contracción. El momento va a ser muy próximo. Debes encontrar un lugar protegido donde alumbrar... más aún sabiendo que en ese momento recuperarás tu verdadera apariencia, que helará de horror y odio a los humanos que te vean.</br>");
		...
		primitives.GD_CreateMsg ("es", "introMan_1", "Eres eres un cazador montaraz, de los pocos habitantes de esta recóndita región de la isla. Vives relativamente cerca de tu pariente lejana, la diosa de la montaña.");
		primitives.GD_CreateMsg ("es", "introMan_2", "Te encuentras en tu cabaña... bueno, en lo que queda de ella: ¿quemada?, ¿pero qué diablos ha sucedido aquí? Miras a tu alrededor y descubres que el incendio fue hace muy poco. Aún queda algún rescoldo humeante.<br/>");
		primitives.GD_CreateMsg ("es", "introMan_3", "¡Qué dolor de cabeza! ¿Qué habrá pasado? Lo último que recuerdas es que fuiste al mundo submarino en búsqueda del anzuelo mágico que necesitaba tu hermano, Hoderi.");
		primitives.GD_CreateMsg ("es", "introMan_4", "Había una bella joven, que se movía con la gracia de los delfines... y luego un rayo lanzado por la diosa de la montaña y la cabaña quemándose.");
		primitives.GD_CreateMsg ("es", "introMan_5", "Mi pariente lejana, en su cueva más allá de las tierras del sur, me tiene que ayudar a descubrir qué pasó.<br/>");
...
usr.transformacionVieja = function() {
	...
	primitives.CA_QuoteBegin ("hechicera", "DLG_" + usr.getVersion() + "_vieja_recibe_diamante", [], false);
...
usr.darComidaAFlautista = function(item1Id) {
	...
	primitives.GD_CreateMsg ("es", "rechaza_comida", "Gracias por traerme algo de comer. Tengo hambre, pero sólo de algo hayas cazado tú.");
	primitives.GD_CreateMsg ("es", "lo_espero_de_otro", "Gracias, aún tengo hambre, pero espero que me lo traiga otra persona.");
	var comida_esperada = "";
...
usr.aceptasPropuesta = function(option, superMsg, superParam, isDialog) {
	...
	*/
	...
	if (typeof option == 'undefined') { // fase 1
		...
		primitives.GD_CreateMsg ("es", "aceptas_S_N", "¿Aceptas?");
		primitives.GD_CreateMsg ("es", "eliges_si", "Sí");
		primitives.GD_CreateMsg ("es", "eliges_no", "No");
